...
class NewSalesReturnDialog extends ConsumerStatefulWidget {
  final int? invoiceId;
  const NewSalesReturnDialog({super.key, this.invoiceId});

  @override
  ConsumerState<NewSalesReturnDialog> createState() => _NewSalesReturnDialogState();
}

class _NewSalesReturnDialogState extends ConsumerState<NewSalesReturnDialog> {
  ...
  Future<void> _loadInvoiceData() async {
    ...
    try {
      ...
      // Load invoice
      final invoice = await (db.select(db.invoices)..where((t) => t.id.equals(widget.invoiceId!))).getSingle();
      ...
      ])..where(db.invoiceItems.invoiceId.equals(widget.invoiceId!));
  ...
  double get _totalAmount {
    return _returnItems.entries.fold(0.0, (sum, item) => sum + (item.key.retailPrice * item.value));
  }
  ...
  Future<void> _submit() async {
    ...
    if (_paymentMethod == 'credit' && _selectedCustomerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يجب اختيار العميل في حالة الآجل')));
      return;
    }
    ...
    try {
      ...
      final itemsToReturn = _returnItems.entries.map((e) {
        return SalesReturnItemsCompanion.insert(
          ...
          productId: e.key.id,
          quantity: e.value,
          unitPrice: e.key.retailPrice,
          total: e.key.retailPrice * e.value,
        );
      }).toList();

      await DbHelpers.processSalesReturn(
        db,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تسجيل مرتجع المبيعات بنجاح'), backgroundColor: AppColors.success));
      }
    } catch (e) {
      if (mounted) {
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    widget.invoiceId != null ? 'إرجاع أصناف من فاتورة مبيعات #${widget.invoiceId}' : 'مرتجع مبيعات جديد',
                                      ...
                                      children: _returnItems.entries.map((e) {
                                        ...
                                        return Card(
                                          ...
                                          child: ListTile(
                                            ...
                                            subtitle: Text('${product.retailPrice} ج.م × $qty = ${product.retailPrice * qty} ج.م', style: TextStyle(color: AppColors.primary, fontFamily: 'Cairo')),
                                            trailing: Row(
                                ...
                                children: [
                                  ...
                                  DropdownButtonFormField<String>(
                                    ...
                                    items: const [
                                      ...
                                      DropdownMenuItem(value: 'credit', child: Text('آجل (خصم من مديونية العميل)')),
                                    ],
                                    onChanged: (val) {
