...
class PurchaseHistoryView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PurchaseHistoryView> createState() => _PurchaseHistoryViewState();
}

class _PurchaseHistoryViewState extends ConsumerState<PurchaseHistoryView> {
  String _searchQuery = '';
  DateTime? _selectedDate;

  void _showInvoiceDetails(PurchaseInvoice invoice, String supplierName) async {
    showDialog(
      context: context,
      builder: (context) => PurchaseInvoiceDetailsDialog(invoice: invoice, supplierName: supplierName),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Map<int, String> supplierMap = {};
    customersAsync.whenData((customers) {
      for (final c in customers) {
        supplierMap[c.id] = c.name;
      }
    });
...
class PurchaseInvoiceDetailsDialog extends ConsumerStatefulWidget {
  final PurchaseInvoice invoice;
  final String supplierName;

  const PurchaseInvoiceDetailsDialog({super.key, required this.invoice, required this.supplierName});

  @override
  ConsumerState<PurchaseInvoiceDetailsDialog> createState() => _PurchaseInvoiceDetailsDialogState();
}

class _PurchaseInvoiceDetailsDialogState extends ConsumerState<PurchaseInvoiceDetailsDialog> {
  ...
  Future<void> _loadItems() async {
    ...
    ])..where(db.purchaseItems.purchaseInvoiceId.equals(widget.invoice.id));
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('المورد: ${widget.supplierName}', style: TextStyle(fontFamily: 'Cairo')),
